 ...
 switch ($action) {
     case 'saisirFrais':
         ...
         case 'validerMajFraisForfait':
            ...
            $vehicule = filter_input(INPUT_POST, 'vehicule', FILTER_DEFAULT, FILTER_FORCE_ARRAY);
        
            if (Utilitaires::lesQteFraisValides($lesFrais)) {
                $pdo->majFraisForfait($idVisiteur, $mois, $lesFrais);
                $pdo->supprimerFraisKilometrique($idVisiteur, $mois);
                if (!empty($vehicule['id']) && is_numeric($vehicule['quantite'])) {
                    $pdo->majFraisForfait($idVisiteur, $mois, [
                        $vehicule['id'] => (int) $vehicule['quantite']
                    ]);
                }
            } else {
